...
void TB_ImageGrayScaler::stim() {
	...
	long size = 0;
	cout << "read image...\n";
	ImageGrayScaler::RGBTriplet_t* rgb_data = bmp2rgb(&width, &height, &size, L"parrot.bmp");
	...
	{
		sc_uint<24> data = (rgb_data[i].red << 0) | (rgb_data[i].green << 8) | (rgb_data[i].blue << 16);
		data_out->write(data);
	
		wait(1);
		wait(1, SC_NS);

		gray_img[3 * i] = data_in->read();
		gray_img[3 * i + 1] = gray_img[3 * i];
		gray_img[3 * i + 2] = gray_img[3 * i];
	}
...
ImageGrayScaler::RGBTriplet_t* TB_ImageGrayScaler::bmp2rgb(int* width, int* height, long* size, LPCTSTR file_name)
{
	...
	*size = bmp_header.bfSize - bmp_header.bfOffBits;
	...
	ImageGrayScaler::RGBTriplet_t* buffer_rgb = new ImageGrayScaler::RGBTriplet_t[*width * *height];
	...
	for (int y = 0; y < *height; y++)
		for (int x = 0; x < 3 * *width; x += 3)
		{
			...
			buffer_rgb[newpos / 3 + 0].red = buffer[bufpos + 2];
			buffer_rgb[newpos / 3 + 1].green = buffer[bufpos + 1];
			buffer_rgb[newpos / 3 + 2].blue = buffer[bufpos];
		}

	cout << "Read bitmap successfully.\n";

	return (ImageGrayScaler::RGBTriplet_t*) buffer_rgb;
}
